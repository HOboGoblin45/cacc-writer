 ...
 */
export async function reviewSection(state) {
    ...
    const { result, durationMs } = await wrapWithTrace(() => _reviewSection(state), 'review_section', { caseId, formType, fieldId });
...
async function _reviewSection(state) {
 ...
 */
function parseReviewResponse(raw, originalDraft) {
    try {
...
function normalizeIssueType(raw) {
    ...
    return valid.includes(s) ? s : 'tone';
}
