...
const PRIMARY_TABS = ['intake','case','workspace','docs','voice'];
const TOOLS_TAB_LABELS = {
  generate:'âš¡ Generate', facts:'ðŸ“‹ Facts', qc:'âœ… QC Grade', docs:'ðŸ“ Docs',
  intel:'ðŸ” Intel', valuation:'ðŸ’° Valuation', memory:'ðŸ§  Memory', pipeline:'ðŸ”„ Pipeline',
  inspect:'ðŸ”¬ Inspect', governance:'ðŸ“œ Governance', learning:'ðŸ“– Learning', system:'âš™ï¸ System'
};
...
function updateActiveCaseBar(caseRecord, meta) {
  ...
  if(acbName) {
    ...
    const borrower = meta?.borrower ? ' Â· ' + meta.borrower : '';
...
async function setActiveFormConfig(formType) {
  ...
  const sub=$('headerSub');
  if(sub)sub.textContent=cfg.label+' â€” Case-based, fact-grounded, Cresci voice';
  ...
  // â”€â”€ Scope enforcement: show/hide deferred banner + disable generate â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ ACTIVE_SCOPE / DEFERRED_SCOPE (populated from /api/forms) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Form picker helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function initFormRegistry() {
  ...
  // â”€â”€ Populate active form picker options â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Populate deferred form picker options â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Sync hidden #newFormType select â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const nft = $('newFormType');
  if(nft) {
    nft.innerHTML = d.forms.map(f => `<option value="${esc(f.id)}">${esc(f.label)}</option>`).join('');
  }

  // â”€â”€ Populate voiceFormType with optgroups â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const vft = $('voiceFormType');
  if(vft) {
...
function renderCaseList(cases) {
  ...
  list.innerHTML=filtered.map(cs=>{
    ...
    const cid="'" + (cs.caseId||'').replace(/'/g,"\\'") + "'", sid="'" + (st||'').replace(/'/g,"\\'") + "'";
    ...
    return `<div class="case-card${isActive?' active':''}" onclick="loadCase(${cid})">
      ...
      <div class="case-card-actions">
        <button class="case-card-action primary" onclick="event.stopPropagation();loadCase(${cid}).then(()=>showTab('workspace'))">âœï¸ Open</button>
        <button class="case-card-action" onclick="event.stopPropagation();loadCase(${cid}).then(()=>showTab('generate'))">âš¡ Generate</button>
        <button class="case-card-action" onclick="event.stopPropagation();loadCase(${cid}).then(()=>{generateAll();})" title="Generate all then insert into ACI">â¤µ Insert</button>
...
async function createCase() {
  ...
  const d=await apiFetch('/api/cases/create',{method:'POST',body:{
    ...
    // â”€â”€ Assignment metadata (new 4-card fields) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function loadCase(caseId) {
  ...
  // â”€â”€ Update active case bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  updateActiveCaseBar(d.caseRecord, d.meta);
  // â”€â”€ Populate all 4 assignment cards â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  populateAssignmentFields(d.meta);
  // â”€â”€ Render metadata chips + workflow badge â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Deferred-form legacy case: show scope warning from server â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function autoFillGenerateInputs(facts, meta) {
  ...
  // â”€â”€ Subject city/area: prefer meta geography, fall back to facts â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function generateAllAndInsertAll() {
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'âš¡ Generatingâ€¦'; }

  setStatus('genStatus', 'Generating ' + fields.length + ' fieldsâ€¦', 'warn');
  ...
  if (!agentOk) {
    if (btn) { btn.disabled = false; btn.textContent = 'âš¡ Generate All + Insert All'; }
    alert('Generation complete! Agent not running â€” please insert manually or start the ACI agent and try again.');
    return;
  }

  // Insert all generated fields
  setStatus('genStatus', 'Inserting all generated sections into ACIâ€¦', 'warn');
  if (btn) btn.textContent = 'âš¡ Insertingâ€¦';
  ...
  if (failedFields.length) {
    ...
    setStatus('genStatus', `âœ“ All ${insertedCount} fields generated and inserted into ACI.`, 'ok');
  }
}
...
function sectionStatusBadge(status) {
  const labels = {
    ...
    copied:      'ðŸ“‹ Copied (paste required)',
    error:       'Error',
  };
...
function makeOutputCard(fieldId, title, text, prompt, opts = {}) {
  ...
  const insertBtn = STATE.caseId ? '<button class="ghost sm insertBtn" onclick="insertField(this)">â†’ ' + agentLabel + '</button>' : '';
...
// ====== PHASE 7 â€” QC REVIEW AUTOMATION ======
...
async function qcRunQC() {
  if (!STATE.caseId) { alert('Select a case first.'); return; }
  setStatus('qcStatus', 'Running QC checksâ€¦', 'warn');
  showErr('qcErrBox', '');
  $('qcSummaryDisplay').innerHTML = '<div class="hint">Running QCâ€¦</div>';
  $('qcFindingsDisplay').innerHTML = '<div class="hint">Running QCâ€¦</div>';
...
async function qcLoadHistory() {
  ...
  wrap.innerHTML = d.runs.map(r => {
    const ts = r.created_at ? new Date(r.created_at).toLocaleString() : 'â€”';
...
function qcRenderSummary(summary) {
  ...
  if (s.clearedSections && s.clearedSections.length) {
    ...
    s.clearedSections.forEach(sec => {
      html += `<span class="chip" style="border-color:var(--ok);color:var(--ok);">âœ“ ${esc(sec)}</span>`;
    });
    html += '</div></div>';
  }
...
// â”€â”€ Drag-and-drop multi-PDF upload â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function processVoiceFiles(files) {
  ...
  for (const file of files) {
    ...
    if (queueEl) {
      ...
      itemEl.innerHTML = `<span style="color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(file.name)}</span><span id="${safeId}" style="color:var(--warn);white-space:nowrap;">uploadingâ€¦</span>`;
    ...
    try {
      ...
      if (data.ok) {
        if (statusEl) { statusEl.textContent = `âœ“ ${(data.extracted || data.extractedCount || 0)} examples`; statusEl.style.color = 'var(--ok)'; }
        successCount++;
      } else {
        if (statusEl) { statusEl.textContent = `âœ— ${data.error || 'failed'}`; statusEl.style.color = 'var(--danger)'; }
        errorCount++;
      }
    } catch (err) {
      const statusEl = document.getElementById(safeId);
      if (statusEl) { statusEl.textContent = err.name === 'AbortError' ? 'âœ— timeout' : 'âœ— error'; statusEl.style.color = 'var(--danger)'; }
      errorCount++;
    }
  }

  if (errorCount === 0) {
    setStatus('voiceImportStatus', `âœ“ ${successCount} file${successCount !== 1 ? 's' : ''} imported successfully.`, 'ok');
...
// â”€â”€ ACI XML upload for voice training â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function processVoiceXmlFiles(files) {
  ...
  for (const file of files) {
    ...
    if (queueEl) {
      const itemEl = document.createElement('div');
      itemEl.style.cssText = 'font-size:11px;padding:4px 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;gap:8px;';
      itemEl.innerHTML = `<span style="color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">ðŸ“‹ ${esc(file.name)}</span><span id="${safeId}" style="color:var(--warn);white-space:nowrap;">parsingâ€¦</span>`;
      queueEl.appendChild(itemEl);
    }
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (statusEl) {
          statusEl.textContent = `âœ“ Case ${data.caseId}${parts.length ? ' Â· ' + parts.join(', ') : ''}`;
  ...
  if (errorCount === 0) {
    setStatus('voiceXmlImportStatus', `âœ“ ${successCount} XML${successCount !== 1 ? 's' : ''} imported. Cases created with full facts + comps.`, 'ok');
  } else if (successCount > 0) {
...
// â”€â”€ Voice Training Library (folder status for all form types) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function refreshVoiceLibrary() {
  ...
  if (el) el.innerHTML = '<span style="color:var(--muted)">Checking foldersâ€¦</span>';
  ...
  for (const result of results) {
    ...
    if (!d.ok || !d.folderExists || !d.total) {
      ...
      const importBtn = newCount > 0
        ...
        : `<span style="font-size:10px;color:var(--ok);">âœ“ all imported</span>`;
      row.innerHTML = `<div style="flex:1;min-width:0;"><div style="font-size:12px;color:var(--text);">${esc(ft.label)}</div><div style="font-size:10px;color:var(--muted);">${d.total} PDF${d.total !== 1 ? 's' : ''} in folder${newCount > 0 ? ` Â· ${newCount} new` : ''}</div></div>${importBtn}`;
    }
    el.appendChild(row);
  }
...
async function bulkImportVoiceFolder(formType, btn) {
  ...
  if (d.ok) {
    ...
    btn.textContent = imported.length ? `âœ“ ${imported.length} done` : 'âœ“ none new';
...
async function checkFolderStatus() {
  ...
  if(!d.total){if(statusEl)statusEl.textContent='voice_pdfs/'+ft+'/ is empty â€” drop PDFs in to get started.';return;}
  const newCount=d.newCount||0;
  const msg=newCount>0
    ?newCount+' new PDF'+(newCount>1?'s':'')+' ready to import ('+d.total+' total in folder)'
    :'All '+d.total+' PDF'+(d.total>1?'s':'')+' already imported â€” drop new ones in to add more';
  ...
  if(scanEl&&d.files&&d.files.length){
    ...
    scanEl.innerHTML=d.files.map(f=>'<span style="display:inline-block;margin:2px 4px 2px 0;font-size:10px;padding:1px 6px;border-radius:999px;border:1px solid '+(f.imported?'rgba(85,209,143,.3)':'rgba(245,200,66,.3)')+';color:'+(f.imported?'var(--ok)':'var(--warn)')+';">'+esc(f.filename)+(f.imported?' âœ“':' NEW')+'</span>').join('');
  }
}
...
async function scanVoiceFolder() {
  ...
  if(errors.length)msg+=' '+errors.length+' error'+(errors.length!==1?'s':'')+': '+errors.map(e=>e.filename+' â€” '+e.error).join('; ');
  setStatus('folderScanStatus',msg,errors.length&&!imported.length?'err':imported.length?'ok':'warn');
  if(imported.length)await loadVoiceExamples();
}

// ====== GENERATE CORE SECTIONS (Phase 2 â€” active scope: 1004 + commercial) ======
...
async function generateCoreSections() {
  ...
  if (isDeferredFormId(STATE.formType)) {
    alert(
      `âš¡ Core Sections is not available for form type "${STATE.formType}".\n` +
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'âš¡ Generatingâ€¦'; }
  setStatus('genStatus', 'Generating core sections (draft + review pass)â€¦', 'warn');
  showErr('genErrBox', '');
  ...
  if (list)  list.innerHTML = '<div class="hint" style="padding:4px 0;">Generatingâ€¦</div>';

  try {
    const d = await apiFetch('/api/cases/' + STATE.caseId + '/generate-core', {
      method:  'POST',
      body:    { twoPass: true },
      timeout: 360000, // 6 min â€” 5 sections Ã— 2-pass each
    });
    ...
    if (btn) { btn.disabled = false; btn.textContent = 'âš¡ Core Sections'; }
  }
}
...
async function insertField(btn) {
  ...
  if (d.ok) {
    ...
    if (agentNotRunning) {
      // Agent not running â€” offer clipboard fallback
 ...
 * Server-side clipboard fallback â€” calls POST /api/cases/:caseId/sections/:fieldId/copy.
 *
...
async function clipboardFallback(fieldId, failureReason) {
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'ðŸ“‹ Copyingâ€¦'; }
  ...
  try {
    ...
    if (!d.ok) {
      ...
      if (btn) { btn.disabled = false; btn.textContent = 'â†’ ' + (STATE.formType === 'commercial' ? 'RQ' : 'ACI'); }
      return;
    }
    ...
    if (btn) {
      btn.textContent = copied ? 'ðŸ“‹ Copied' : 'ðŸ“‹ Copy failed';
      btn.classList.add('ok');
      btn.disabled = false;
      setTimeout(() => {
        btn.textContent = 'â†’ ' + (STATE.formType === 'commercial' ? 'RQ' : 'ACI');
        btn.classList.remove('ok');
      }, 4000);
    }
...
async function checkAgentStatus() {
  ...
  try {
    ...
    if (h.ok) {
      ...
      if (label) label.textContent = openaiReady ? 'OpenAI âœ“' : (h.aiKeySet ? 'OpenAI âš ' : 'OpenAI âœ—');
...
// â”€â”€ Workflow status label + CSS class map â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const WF_LABELS = {
  facts_incomplete:       'Facts Incomplete',
  ready_for_generation:   'Ready to Generate',
  generation_in_progress: 'Generatingâ€¦',
  sections_drafted:       'Sections Drafted',
  awaiting_review:        'Awaiting Review',
  automation_ready:       'Automation Ready',
  insertion_in_progress:  'Insertingâ€¦',
  verified:               'Verified âœ“',
...
function showMissingFactsPanel(warnings, pendingFields) {
  ...
  list.innerHTML=warnings.map(w=>{
    ...
    const icon=w.severity==='required'?'âœ—':'â—‹';
...
// DESKTOP PRODUCTION PHASE â€” Health Panel, Version Display, Export Bundle
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
async function loadHealthStatus() {
  ...
  try {
    ...
    // Server unreachable â€” mark server offline, others unknown
...
async function createSupportBundle() {
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'â³ Exporting...'; }
  try {
    ...
    if (d.ok) {
      ...
      showExportToast('âœ“ Bundle created: ' + name, false, 5000);
    } else {
      showExportToast('âœ— Export failed: ' + (d.error || 'unknown error'), true);
    }
  } catch (err) {
    showExportToast('âœ— Export error: ' + err.message, true);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = 'â¬‡ Export Bundle'; }
...
async function generateCompCommentary() {
  ...
  // â”€â”€ UI: loading state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (statusEl) { statusEl.style.display = 'block'; statusEl.className = 'status warn'; statusEl.textContent = 'Generating commentaryâ€¦'; }
  if (chipEl)   { chipEl.style.display = 'inline-flex'; chipEl.className = 'chip warn'; chipEl.textContent = 'Generatingâ€¦'; }
  ...
  try {
    ...
    // â”€â”€ Render output text â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    const text = d.text || '';
    if (outArea) { outArea.textContent = text; outArea.classList.add('visible'); }

    // â”€â”€ Render metadata row (sources, phrases, adj summary) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (outMeta) {
      const parts = [];
      ...
      outMeta.textContent = parts.join(' Â· ');
      outMeta.style.display = parts.length ? 'block' : 'none';
    }

    // â”€â”€ Show action buttons â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Status chip â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function insertCompCommentary() {
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'Insertingâ€¦'; }
...
/** Phase â†’ progress % mapping (approximate) */
const FD_PHASE_PCT = {
  pending:    2,
  context:    10,
  plan:       18,
...
/**
 * generateFullDraft()
 * Triggered by the "âš¡ Full Draft" button.
...
async function generateFullDraft() {
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'âš¡ Startingâ€¦'; }
  setStatus('genStatus', 'Starting full-draft orchestratorâ€¦', 'warn');
  showErr('genErrBox', '');
  ...
  _fdResetPanel();
  _fdSetPhase('context');
  _fdSetProgress(2, 'Building assignment contextâ€¦');
  ...
  try {
    ...
    if (!d.ok || !d.runId) {
      ...
      if (btn) { btn.disabled = false; btn.textContent = 'âš¡ Full Draft'; }
      return;
    }
    ...
    setStatus('genStatus', 'Full draft runningâ€¦ (run: ' + d.runId + ')', 'warn');
    if (btn) { btn.disabled = false; btn.textContent = 'âš¡ Full Draft'; }
...
async function _fdPollOnce() {
  ...
  } catch { /* non-fatal â€” keep polling */ }
}
 ...
 *
 * @param {object} status â€” response from GET /api/generation/runs/:runId/status
 */
function renderFullDraftProgress(status) {
  ...
  // â”€â”€ Section rows â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function _fdOnComplete(status) {
  ...
  // Final progress bar
  _fdSetProgress(ok ? 100 : 100, ok ? 'Complete âœ“' : 'Finished with errors');
  ...
  if (warnWrap) {
    if (warnings.length) {
      warnWrap.innerHTML = warnings.map(w =>
        `<div class="fd-warning-item">âš  ${esc(typeof w === 'string' ? w : (w.message || JSON.stringify(w)))}</div>`
      ).join('');
...
function renderRunMetrics(phaseTimings, retrieval, status) {
  ...
  grid.innerHTML = metrics.map(m =>
    `<div class="fd-metric-item">
      ...
      <span class="fd-metric-value${m.cls ? ' ' + m.cls : ''}">${esc(m.value || 'â€”')}</span>
    </div>`
  ).join('');
...
function _fdPhaseLabel(phase, done, total) {
  const labels = {
    pending:    'Startingâ€¦',
    context:    'Building assignment contextâ€¦',
    plan:       'Planning report sectionsâ€¦',
    retrieval:  'Retrieving narrative memoryâ€¦',
    analysis:   'Running analysis jobsâ€¦',
    drafting:   total > 0 ? `Drafting sections (${done}/${total})â€¦` : 'Drafting sectionsâ€¦',
    running:    total > 0 ? `Drafting sections (${done}/${total})â€¦` : 'Drafting sectionsâ€¦',
    validation: 'Validating draftâ€¦',
    assembly:   'Assembling draft packageâ€¦',
...
// ====== GENERATE TAB â€” COMMAND STRIP + INSPECTOR ============================
 ...
 *
 * @param {object} status â€” run status from GET /api/generation/runs/:runId/status
 */
function _updateInspector(status) {
  ...
  // â”€â”€ Run Status card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Memory Used card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Warnings card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const warnings  = status.warnings || [];
...
function renderIntelligence(b) {
  ...
  // Flags â€” show active flags with green, inactive grayed
  const flags = b.flags || {};
...
async function uploadDocument() {
  ...
  if (!fileInput.files.length) return alert('Choose one or more PDF files.');
  const files = Array.from(fileInput.files);
  const docType = typeSelect.value;
  statusEl.innerHTML = `<span style="color:var(--ds-accent,#4af);">Uploading ${files.length} file${files.length>1?'s':''}...</span>`;
  const results = [];
  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    statusEl.innerHTML = `<span style="color:var(--ds-accent,#4af);">Processing ${i+1}/${files.length}: ${esc(file.name)}...</span>`;
    const fd = new FormData();
    fd.append('file', file);
    if (docType) fd.append('docType', docType);
    try {
      const r = await fetch(server() + `/api/cases/${STATE.caseId}/documents/upload`, { method: 'POST', body: fd });
      const d = await r.json();
      if (!d.ok) { results.push(`<span style="color:var(--ds-red,#f44);">âœ— ${esc(file.name)}: ${esc(d.error||'Failed')}</span>`); continue; }
      results.push(
        `<span style="color:var(--ds-green,#4c6);">âœ“ ${esc(file.name)}: ${esc(d.classification?.label||d.docType||'uploaded')}</span>` +
        `<span style="font-size:11px;color:var(--muted);"> â€” ${d.wordCount} words, ${d.pageCount} pages` +
        (d.extraction ? `, ${d.extraction.factsExtracted} facts` : '') + `</span>`
      );
    } catch (e) {
      results.push(`<span style="color:var(--ds-red,#f44);">âœ— ${esc(file.name)}: ${esc(e.message)}</span>`);
    }
  }
...
async function loadDocuments() {
  ...
  try {
    const r = await fetch(`/api/cases/${STATE.caseId}/documents`);
    const d = await r.json();
...
async function loadExtractionSummary() {
  ...
  try {
    const r = await fetch(`/api/cases/${STATE.caseId}/extraction-summary`);
    const d = await r.json();
...
async function loadExtractedFacts(status) {
  ...
  try {
    const url = status ? `/api/cases/${STATE.caseId}/extracted-facts?status=${status}` : `/api/cases/${STATE.caseId}/extracted-facts`;
...
async function mergeSelectedFacts() {
  ...
  try {
    const r = await fetch(`/api/cases/${STATE.caseId}/extracted-facts/merge`, {
...
async function reviewFactAction(factId, action) {
  if (!STATE.caseId) return;
  try {
    await fetch(`/api/cases/${STATE.caseId}/extracted-facts/review`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function loadExtractedSections(status) {
  ...
  try {
    const url = status ? `/api/cases/${STATE.caseId}/extracted-sections?status=${status}` : `/api/cases/${STATE.caseId}/extracted-sections`;
    const r = await fetch(url);
    const d = await r.json();
...
async function approveSectionAction(sectionId) {
  if (!STATE.caseId) return;
  try {
    const r = await fetch(`/api/cases/${STATE.caseId}/extracted-sections/${sectionId}/approve`, { method: 'POST' });
    const d = await r.json();
    if (!d.ok) throw new Error(d.error);
    loadExtractedSections('pending');
    loadExtractionSummary();
...
async function rejectSectionAction(sectionId) {
  if (!STATE.caseId) return;
  try {
    const r = await fetch(`/api/cases/${STATE.caseId}/extracted-sections/${sectionId}/reject`, { method: 'POST' });
    const d = await r.json();
    if (!d.ok) throw new Error(d.error);
    loadExtractedSections('pending');
    loadExtractionSummary();
...
async function reExtractDoc(docId) {
  if (!STATE.caseId) return;
  try {
    const r = await fetch(`/api/cases/${STATE.caseId}/documents/${docId}/extract`, { method: 'POST' });
    const d = await r.json();
    if (!d.ok) throw new Error(d.error);
...
async function deleteDoc(docId) {
  ...
  try {
    const r = await fetch(`/api/cases/${STATE.caseId}/documents/${docId}`, { method: 'DELETE' });
    const d = await r.json();
    if (!d.ok) throw new Error(d.error);
    loadDocuments();
    loadExtractionSummary();
...
// ====== PHASE 6 â€” MEMORY, VOICE & RETRIEVAL UI ==============================
...
// â”€â”€ Memory Summary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Approved Memory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function _memApprovedCard(item) {
  ...
  const quality = item.qualityScore != null ? item.qualityScore : 'â€”';
  ...
  return `<div class="mem-item" style="padding:8px 0;border-bottom:1px solid var(--border);" data-mem-id="${id}">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
      <div style="flex:1;">
        ...
        <div style="font-size:11px;margin-top:4px;color:var(--text);line-height:1.4;font-family:var(--mono);opacity:.85;">${preview}${(item.text || '').length > 200 ? 'â€¦' : ''}</div>
        ${tags ? '<div style="margin-top:3px;">' + tags + '</div>' : ''}
      </div>
      <div style="display:flex;gap:4px;flex-shrink:0;">
        <button class="ghost sm" onclick="memToggleActive('${id}',${active ? 0 : 1})" title="${active ? 'Deactivate' : 'Activate'}">${active ? 'â¸' : 'â–¶'}</button>
        <button class="ghost sm" onclick="memEditApproved('${id}')" title="Edit">âœŽ</button>
      </div>
    </div>
  </div>`;
}
...
// â”€â”€ Staged Candidates â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return `<div class="mem-staged-item" style="padding:8px 0;border-bottom:1px solid var(--border);" data-staged-id="${id}">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
      ...
      ${isPending ? `<div style="display:flex;gap:4px;flex-shrink:0;">
        ...
        <button class="ghost sm" style="font-size:10px;padding:2px 8px;" onclick="memEditBeforeApprove('${id}')" title="Edit then approve">âœŽ</button>
      </div>` : ''}
    </div>
  </div>`;
}
...
// â”€â”€ Comparable Commentary Memory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Voice Profile â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Retrieval Preview â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function memPreviewRetrieval() {
  ...
  el.innerHTML = '<div class="hint">Loading retrieval previewâ€¦</div>';
  try {
    ...
    if (pack.metadata) html += ' â€” scanned ' + (pack.metadata.totalScanned || 0) + ' items in ' + (pack.metadata.durationMs || 0) + 'ms';
    html += '</div>';
    ...
    for (const ex of examples) {
      const score = ex.score != null ? (typeof ex.score === 'object' ? ex.score.totalScore : ex.score) : 'â€”';
      ...
      html += `<div style="padding:6px 0;border-bottom:1px solid var(--border);">
        ...
        <div style="font-size:10px;margin-top:3px;font-family:var(--mono);opacity:.8;">${esc((ex.text || '').slice(0, 150))}${(ex.text || '').length > 150 ? 'â€¦' : ''}</div>
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   Phase 10 â€” Business Operations Layer (Timeline, Archive, Export, Health)
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
async function loadCaseTimeline() {
  ...
  if (!card || !body) return;
  card.style.display = '';
  body.innerHTML = '<div class="hint">Loading timelineâ€¦</div>';
  try {
    ...
    for (const ev of events) {
      ...
      html += '<div style="font-size:10px;color:var(--gen-fg-muted);margin-top:2px;">' + esc(cat) + ' Â· ' + esc(tsShort) + '</div>';
      html += '</div></div>';
    }
    html += '</div>';
    body.innerHTML = html;
...
// â”€â”€ Case Business Status â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Case Readiness Checklist â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Missing-Facts Dashboard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Due-Date Queue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Pipeline Summary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Show business cards when case loads â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Enhanced section governance card for workspace assistant â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Comp Grid Status â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Comp Queue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Burden Metrics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Adjustment Notebook â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Sales Comparison Summary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Income Approach â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Cost Approach â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Learning Dashboard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function learnLoadDashboard() {
  ...
  // â”€â”€ Acceptance / Rejection Metrics â”€â”€
  ...
  html += _learnStat(acceptRate != null ? acceptRate + '%' : 'â€”', 'Accept Rate', acceptRate > 70 ? 'good' : acceptRate > 40 ? 'mid' : 'low');
  html += _learnStat(modRate != null ? modRate + '%' : 'â€”', 'Modify Rate', 'mid');
  html += `</div>`;
  ...
  // â”€â”€ Revision Diff Stats (current case) â”€â”€
...
// â”€â”€ Why This Suggestion (for workspace) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function workspaceLoadWhySuggestion(fieldId) {
  ...
  if (influence && influence.revisionStats) {
    ...
    html += `<div style="font-size:10px;margin-top:6px;color:var(--muted);">` +
      `Avg change ratio: <strong>${rs.averageChangeRatio != null ? Math.round(rs.averageChangeRatio * 100) + '%' : 'â€”'}</strong> | ` +
...
// â”€â”€ Memory Health Tools â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Phase 6: System Reliability â€” Backup Scheduler, Restore Verification, Audit Log
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
// â”€â”€ System Health â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Backup Schedule â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function sysLoadSchedule() {
  ...
  el.innerHTML =
    `<div style="font-size:11px;">` +
      ...
      `<div>Every <strong>${sched.interval_hours || sched.intervalHours || 24}h</strong> Â· Keep <strong>${sched.retention_days || sched.retentionDays || 30} days</strong> Â· Max <strong>${sched.max_backups || sched.maxBackups || 10}</strong></div>` +
    `</div>`;
}
...
// â”€â”€ Backup History â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function sysLoadBackups() {
  ...
  el.innerHTML = '<div class="bkp-list">' + backups.map(b => {
    ...
    const sizeMB = b.file_size_bytes ? (b.file_size_bytes / (1024 * 1024)).toFixed(1) + ' MB' : 'â€”';
    const date = b.created_at ? new Date(b.created_at).toLocaleString() : 'â€”';
    const id = b.id || '';
    return (
      `<div class="bkp-item">` +
        `<div class="bkp-item-icon">${status === 'verified' ? '&#x2705;' : '&#x1F4BE;'}</div>` +
        `<div class="bkp-item-info">` +
          `<div class="bkp-item-name">${esc(b.backup_type || 'manual')} â€” ${esc(date)}</div>` +
          `<div class="bkp-item-meta">${esc(sizeMB)} Â· ${esc(b.file_hash ? b.file_hash.slice(0, 12) + '...' : 'no hash')}</div>` +
        `</div>` +
...
async function sysVerifyBackup(backupId) {
  ...
  if (res && !res.error) {
    ...
    setStatus('sysBackupStatus', valid ? 'Backup verified â€” integrity OK.' : 'Verification failed: ' + (res.reason || 'corrupt'), valid ? 'ok' : 'err');
    sysLoadBackups();
  } else {
...
// â”€â”€ Restore Verification Workflow â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function sysStartRestore(backupId) {
  ...
  const sizeMB = backup.file_size_bytes ? (backup.file_size_bytes / (1024 * 1024)).toFixed(1) + ' MB' : 'â€”';
  const date = backup.created_at ? new Date(backup.created_at).toLocaleString() : 'â€”';
...
// â”€â”€ Disaster Recovery Status â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Audit Log Viewer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function sysLoadAuditLog() {
  ...
  el.innerHTML = '<div class="audit-log-list">' + events.map(e => {
    ...
    const caseLabel = e.case_id ? ` Â· Case ${esc(e.case_id.slice(0, 8))}` : '';
    return (
...
function _sysRenderAuditPager(total) {
  ...
  pager.innerHTML =
    ...
    `<span style="font-size:10px;color:var(--muted);padding:4px;">Showing ${_sysAuditOffset + 1}â€“${Math.min(_sysAuditOffset + _sysAuditLimit, total)} of ${total}</span>` +
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Phase 7: Inspection Capture â€” Photo Upload, Checklists, Measurements, Conditions
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
// â”€â”€ Inspection List â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function inspLoadList() {
  ...
  el.innerHTML = items.map(i => {
    ...
    const date = i.scheduled_date ? new Date(i.scheduled_date).toLocaleDateString() : 'â€”';
...
// â”€â”€ Checklist Templates â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Photo Capture â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  el.innerHTML = photos.map(p => {
    ...
    return `<div class="photo-card">` +
      ...
      `<div class="photo-card-info">` +
        ...
        `<div class="photo-card-meta">${esc(p.file_name || (isUrl ? 'URL link' : 'â€”'))}</div>` +
...
// â”€â”€ Condition Findings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Measurements â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function inspLoadMeasurements() {
  ...
  el.innerHTML = items.map(m => {
    const area = (m.length_ft && m.width_ft) ? (m.length_ft * m.width_ft).toFixed(0) : 'â€”';
    return `<div class="meas-row">` +
      `<span class="meas-row-label">${esc(m.room_name || m.label || '')}</span>` +
      `<span class="meas-row-dim">${m.length_ft || 'â€”'} Ã— ${m.width_ft || 'â€”'}</span>` +
...
// â”€â”€ Inspection Summary & Push to Context â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function inspBuildSummary() {
  ...
  html += `<div><strong>Type:</strong> ${esc(s.inspection_type || 'â€”')}</div>`;
  html += `<div><strong>Status:</strong> ${esc(s.status || 'â€”')}</div>`;
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Phase 7: Insertion Reliability Panel â€” Readback Verification & Replay
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
async function insLoadReliability() {
  ...
  html += `<div>Run: <strong>${esc(run.id?.slice(0, 12) || '')}</strong> Â· ${esc(run.status || 'â€”')}</div>`;
  html += `<div style="margin-top:4px;">`;
  html += `<span style="color:var(--ok);font-weight:700;">${verified} verified</span> Â· `;
  html += `<span style="color:var(--danger);font-weight:700;">${failed} failed</span> Â· `;
...
async function insShowRunHistory() {
  ...
  el.innerHTML = '<div style="font-size:10px;font-weight:700;margin-bottom:6px;text-transform:uppercase;color:var(--muted);">Run History</div>' +
    runs.map(r => {
      const date = r.created_at ? new Date(r.created_at).toLocaleString() : 'â€”';
      const dest = r.destination || r.target_software || 'â€”';
      return `<div class="ins-run-item">` +
        ...
        `<span class="ins-field-status ${r.status === 'completed' ? 'verified' : r.status === 'failed' ? 'failed' : 'pending'}">${esc(r.status || 'â€”')}</span>` +
      `</div>`;
    }).join('');
}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Phase 7: Pipeline Enhancements â€” Fact Cards, Verification Queue, Dedup
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
// â”€â”€ Extracted Fact Cards rendering (enhances existing dpRenderExtractedData) â”€
...
function dpRenderFactCards(data, source) {
  ...
  el.innerHTML = entries.map(item => {
    const field = item.field || item.field_name || item.key || 'â€”';
...
// â”€â”€ Duplicate Detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (!dupes.length) {
    ...
    el.innerHTML = '<div style="font-size:10px;font-weight:700;color:var(--warn);margin-bottom:4px;">Potential Duplicates:</div>' +
      dupes.map(([field, count]) =>
        `<div style="font-size:11px;padding:2px 0;"><strong>${esc(field)}</strong> â€” ${count} entries</div>`
      ).join('');
  }
}

// â”€â”€ Verification Queue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function dpLoadVerificationQueue() {
  ...
  el.innerHTML = items.slice(0, 20).map(f => {
    const field = f.field_name || f.key || 'â€”';
    const value = f.value ?? 'â€”';
    return `<div class="fact-card">` +
      `<div class="fact-card-head">` +
        `<span class="fact-card-field">${esc(String(field))}</span>` +
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Phase 8: Golden-Path Validation â€” In-Browser E2E Test Runner
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Phase 9: Unified Valuation Desk â€” Comp Grid, Candidates, Adjustments,
...
// â”€â”€ Comp Candidate Queue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function valRenderCandidates(intel) {
  ...
  el.innerHTML = sorted.map(c => {
    ...
    const addr = c.candidate?.address || c.sourceKey || 'â€”';
    ...
    return `<div class="val-cand">` +
      ...
      `<div style="flex:1;">` +
        ...
        `<div style="font-size:10px;color:var(--muted);">${priceStr} Â· Tier ${tier} Â· ${esc(status)}</div>` +
...
// â”€â”€ Comp Grid â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function valLoadGrid() {
  ...
  features.forEach((feat, fi) => {
    ...
    html += `<tr><td class="val-grid-label">${esc(feat)}</td><td class="val-grid-value">â€”</td>`;
    gridSlots.forEach(s => {
      const data = s.candidateData || s.candidate_data || s.candidate || {};
      const parsed = typeof data === 'string' ? JSON.parse(data) : data;
      let val = parsed[key] || parsed[key.replace(/([A-Z])/g, '_$1').toLowerCase()] || 'â€”';
      if (key === 'salePrice' && val !== 'â€”') val = '$' + Number(val).toLocaleString();
  ...
  html += '<tr style="border-top:2px solid var(--gold);"><td class="val-grid-label" style="color:var(--gold);font-weight:900;">Indicated Value</td><td>â€”</td>';
  gridSlots.forEach(s => {
    const iv = s.indicated_value || s.indicatedValue || 'â€”';
    html += `<td class="val-grid-value" style="color:var(--gold);">${iv !== 'â€”' ? '$' + Number(iv).toLocaleString() : 'â€”'}</td>`;
  });
...
async function valLoadGridSummary() {
  ...
  el.innerHTML = `<div style="font-size:11px;">` +
    `<div>Range: <strong>$${Number(range.low || 0).toLocaleString()}</strong> â€“ <strong>$${Number(range.high || 0).toLocaleString()}</strong></div>` +
    `<div>Average: <strong>$${Number(res.average || 0).toLocaleString()}</strong></div>` +
    `<div>Slots filled: <strong>${res.filledSlots || 0}/6</strong></div>` +
  `</div>`;
}

// â”€â”€ Adjustment Support Notebook â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function valLoadAdjustments() {
  ...
  el.innerHTML = ADJ_CATEGORIES.map(cat => {
    ...
    const subj = adj.subject_value ?? adj.subjectValue ?? 'â€”';
    const comp = adj.comp_value ?? adj.compValue ?? 'â€”';
    const suggested = adj.suggested_amount ?? adj.suggestedAmount ?? 'â€”';
    const final = adj.final_amount ?? adj.finalAmount ?? 'â€”';
...
// â”€â”€ Burden & Contradiction Visibility â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function valLoadBurden() {
  ...
  el.innerHTML = slots.map(s => {
    ...
    const addr = s.candidateData?.address || s.candidate_data?.address || 'â€”';
    return `<div style="margin-bottom:8px;padding:8px 10px;border:1px solid var(--border);border-radius:8px;font-size:11px;">` +
      `<div style="font-weight:700;">Slot ${slotNum}: ${esc(typeof addr === 'string' ? addr : 'â€”')}</div>` +
      ...
      cEl.innerHTML = '<div style="font-size:10px;font-weight:700;text-transform:uppercase;color:var(--danger);margin-bottom:4px;">Contradictions</div>' +
        contradictions.map(c =>
          `<div style="font-size:11px;padding:4px 0;border-bottom:1px solid rgba(255,255,255,.03);">` +
            `<strong>${esc(c.field || c.category || 'â€”')}</strong>: ${esc(c.message || c.description || '')}` +
          `</div>`
        ).join('');
    }
  }
...
// â”€â”€ Income Approach â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function valLoadIncome() {
  ...
  const rows = [
    ...
    ['Operating Expenses', d.expenses_json ? (typeof d.expenses_json === 'string' ? '(see detail)' : Object.values(d.expenses_json).reduce((a, b) => a + (Number(b) || 0), 0)) : 'â€”'],
    ['Net Income', d.net_income || d.netIncome],
    ['Indicated Value', d.indicated_value || d.indicatedValue],
  ];
  rows.forEach(([label, val]) => {
    const v = val != null && val !== 'â€”' ? (typeof val === 'number' && val > 100 ? '$' + Number(val).toLocaleString() : val) : 'â€”';
  ...
  if (rentComps.length) {
    ...
    rentComps.forEach(rc => {
      html += `<div style="font-size:10px;padding:2px 0;">${esc(rc.address || 'â€”')} â€” $${Number(rc.monthlyRent || rc.monthly_rent || 0).toLocaleString()}/mo</div>`;
    });
  }
  el.innerHTML = html;
...
// â”€â”€ Cost Approach â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function valLoadCost() {
  ...
  let html = '<div>';
  rows.forEach(([label, val]) => {
    const v = val != null ? '$' + Number(val).toLocaleString() : 'â€”';
...
// â”€â”€ Reconciliation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function valLoadReconciliation() {
  ...
  if (summaryEl) {
    ...
    summaryEl.innerHTML = approaches.map(a => {
      const v = a.value ? '$' + Number(a.value).toLocaleString() : 'â€”';
      const w = a.weight != null ? (a.weight * 100).toFixed(0) + '%' : 'â€”';
  ...
  // Narrative â€” sync both reconciliation textareas
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Phase 10: Governance Dashboard â€” Section Freshness, Missing Facts,
...
// â”€â”€ Case Status & Pipeline â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function govLoadCaseStatus() {
  ...
  const workflow = meta.workflow_status || meta.workflowStatus || 'â€”';
  ...
  el.innerHTML = `<div style="font-size:12px;">` +
    ...
    `<div style="display:flex;justify-content:space-between;"><span style="color:var(--muted);">Address</span><strong>${esc(meta.address || 'â€”')}</strong></div>` +
  `</div>`;
  ...
  if (pEl) {
    ...
    pEl.innerHTML = '<div class="gov-pipeline-strip">' +
      ...
      }).join('<span style="color:var(--muted);font-size:10px;">â€º</span>') +
    '</div>';
  }
}
...
async function govAdvancePipeline(nextStage) {
  ...
  if (res && !res.error) {
    setStatus('govPipelineStatus', `Pipeline â†’ ${nextStage}`, 'ok');
    govLoadCaseStatus();
  } else {
...
// â”€â”€ Section Governance â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function govRenderSections() {
  ...
  el.innerHTML = sections.map(s => {
    const id = s.section_id || s.sectionId || s.id || 'â€”';
    ...
    const qualStr = quality != null ? (quality * 100).toFixed(0) + '%' : 'â€”';
    ...
    return `<div class="gov-section ${esc(freshness)}" onclick="govShowSectionDetail('${esc(id)}')">` +
      `<div style="flex:1;">` +
        ...
        `<div style="font-size:10px;color:var(--muted);">${esc(profile)}${regenCount ? ' Â· regen Ã—' + regenCount : ''}${staleReason ? ' Â· ' + esc(staleReason) : ''}</div>` +
      `</div>` +
...
async function govShowSectionDetail(sectionId) {
  ...
  const freshness = s.freshness_status || s.freshnessStatus || 'not_generated';
  const quality = s.quality_score || s.qualityScore;
  const promptVer = s.prompt_version || s.promptVersion || 'â€”';
  const staleReason = s.stale_reason || s.staleReason || 'â€”';
  const staleSince = s.stale_since || s.staleSince || 'â€”';
  const regenCount = s.regeneration_count || s.regenerationCount || 0;
  const generatedAt = s.generated_at || s.generatedAt || 'â€”';
  ...
  html += `<div style="display:flex;justify-content:space-between;padding:3px 0;"><span style="color:var(--muted);">Quality Score</span><strong>${quality != null ? (quality * 100).toFixed(0) + '%' : 'â€”'}</strong></div>`;
  ...
  if (depSnapshot && typeof depSnapshot === 'object') {
    ...
    paths.slice(0, 12).forEach(p => {
      ...
      html += `<div style="display:flex;justify-content:space-between;padding:2px 0;font-size:10px;"><span class="gov-fact-path">${esc(p)}</span><span style="font-weight:600;">${esc(String(val != null ? val : 'â€”')).slice(0, 40)}</span></div>`;
    });
    if (paths.length > 12) html += `<div style="font-size:10px;color:var(--muted);">â€¦and ${paths.length - 12} more</div>`;
  }
...
// â”€â”€ Freshness Summary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function govLoadFreshness() {
  ...
  el.innerHTML = `<div class="gov-donut">` +
    ...
    `<div class="gov-donut-labels">` +
      `<div><span style="color:var(--ok);">â—</span> Current: <strong>${current}</strong></div>` +
      `<div><span style="color:var(--danger);">â—</span> Stale: <strong>${stale}</strong></div>` +
      `<div><span style="color:var(--gold);">â—</span> Regenerating: <strong>${regenerating}</strong></div>` +
      `<div><span style="color:var(--muted);">â—</span> Not Generated: <strong>${notGenerated}</strong></div>` +
      `<div style="margin-top:3px;font-weight:700;">Total: ${total}</div>` +
    `</div>` +
  `</div>`;
}

// â”€â”€ Missing Facts Dashboard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function govLoadMissingFacts() {
  ...
  entries.forEach(entry => {
    const fieldId = entry.fieldId || entry.field_id || entry.sectionId || 'â€”';
...
// â”€â”€ Dependency Graph â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (Array.isArray(entries)) {
    entries.forEach(entry => {
      ...
      if (upstream.length) {
        ...
        html += upstream.map(u => `<span class="gov-dep-node">${esc(String(u).replace(/_/g, ' '))}</span>`).join('<span class="gov-dep-arrow">â†’</span>');
        html += ` <span class="gov-dep-arrow">â†’</span> <span class="gov-dep-node" style="border-color:var(--gold);">${esc(label)}</span>`;
      }

      if (downstream.length) {
        html += `<div style="font-size:10px;color:var(--muted);margin-top:3px;margin-bottom:2px;">Feeds into:</div>`;
        html += `<span class="gov-dep-node" style="border-color:var(--gold);">${esc(label)}</span> <span class="gov-dep-arrow">â†’</span> `;
...
// â”€â”€ Generation Runs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  el.innerHTML = runs.slice(0, 10).map(r => {
    ...
    return `<div class="gov-run-item">` +
      ...
      `<div style="font-size:10px;margin-top:3px;">` +
        `${success}/${total} sections${errors ? ` Â· <span style="color:var(--danger);">${errors} errors</span>` : ''}` +
        `${elapsed ? ` Â· ${(elapsed / 1000).toFixed(1)}s` : ''}` +
      `</div>` +
    `</div>`;
  }).join('');
}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// Phase 11: Learning Dashboard â€” Patterns, Suggestions, Explainability,
...
// â”€â”€ Suggestion Acceptance Metrics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function lrnLoadAcceptanceRate() {
  ...
  const accRate = d.acceptanceRate != null ? (d.acceptanceRate * 100).toFixed(0) : 'â€”';
  const modRate = d.modificationRate != null ? (d.modificationRate * 100).toFixed(0) : 'â€”';
...
// â”€â”€ Case Learning Report â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function lrnLoadCaseReport() {
  ...
  if (archive) {
    html += `<div style="margin-bottom:6px;">` +
      `<div style="display:flex;justify-content:space-between;"><span style="color:var(--muted);">Form</span><strong>${esc(archive.formType || 'â€”')}</strong></div>` +
      `<div style="display:flex;justify-content:space-between;"><span style="color:var(--muted);">Property</span><strong>${esc(archive.propertyType || 'â€”')}</strong></div>` +
      `<div style="display:flex;justify-content:space-between;"><span style="color:var(--muted);">Archived</span><strong>${esc(String(archive.archivedAt || 'â€”').slice(0, 10))}</strong></div>` +
    `</div>`;
  }
  ...
  if (sugg) {
    ...
    html += `<div style="display:flex;justify-content:space-between;"><span>Rate</span><strong>${sugg.acceptanceRate != null ? (sugg.acceptanceRate * 100).toFixed(0) + '%' : 'â€”'}</strong></div>`;
  }
  ...
  if (rev) {
    ...
    html += `<div style="display:flex;justify-content:space-between;"><span>Avg Change</span><strong>${rev.averageChangeRatio != null ? (rev.averageChangeRatio * 100).toFixed(0) + '%' : 'â€”'}</strong></div>`;
  }
...
// â”€â”€ Learned Patterns â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function lrnLoadPatterns() {
  ...
  el.innerHTML = patterns.slice(0, 50).map(p => {
    ...
    const type = p.pattern_type || p.patternType || 'â€”';
    ...
    return `<div class="lrn-pattern" onclick="lrnShowPatternDetail('${esc(p.id)}')">` +
      ...
      `<div style="flex:1;">` +
        ...
        `<div style="font-size:10px;color:var(--muted);">Used ${usage}Ã— ${lastUsed ? 'Â· last ' + esc(String(lastUsed).slice(0, 10)) : ''}</div>` +
      `</div>` +
...
async function lrnShowPatternDetail(patternId) {
  ...
  const type = pattern.pattern_type || pattern.patternType || 'â€”';
  ...
  if (srRes && !srRes.error) {
    html += `<div style="display:flex;justify-content:space-between;padding:3px 0;"><span style="color:var(--muted);">Success Rate</span><strong>${srRes.successRate != null ? (srRes.successRate * 100).toFixed(0) + '%' : 'â€”'}</strong></div>`;
...
// â”€â”€ Suggestion History â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function lrnLoadSuggestionHistory() {
  ...
  el.innerHTML = outcomes.slice(0, 30).map(o => {
    ...
    const section = o.section_id || o.sectionId || 'â€”';
    const type = o.suggestion_type || o.suggestionType || 'â€”';
    ...
    return `<div class="lrn-sugg-row">` +
      ...
      `<div style="flex:1;">` +
        `<div style="font-weight:700;">${esc(label)}</div>` +
        `<div style="font-size:10px;color:var(--muted);">${esc(type.replace(/_/g, ' '))}${reason ? ' Â· ' + esc(reason) : ''}</div>` +
      `</div>` +
    `</div>`;
  }).join('');
}

// â”€â”€ Revision Diff Stats â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function lrnLoadDiffStats() {
  ...
  const avg = res.averageChangeRatio != null ? (res.averageChangeRatio * 100).toFixed(0) : 'â€”';
  ...
  if (most.length) {
    ...
    most.forEach(m => {
      ...
      const label = (m.sectionId || m.section_id || 'â€”').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
...
// â”€â”€ Influence Explainability â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function lrnLoadInfluence() {
  ...
  if (factors.length) {
    ...
    factors.forEach(f => {
      html += `<div class="lrn-influence-factor">` +
        `<span style="font-weight:700;">${esc(f.factor || f.name || 'â€”')}</span>` +
        `<span style="color:var(--muted);">${esc(f.description || '')}</span>` +
        `<span style="font-family:var(--mono);font-weight:700;">${f.weight != null ? (f.weight * 100).toFixed(0) + '%' : 'â€”'}</span>` +
      `</div>`;
    });
  }
  ...
  if (topPatterns.length) {
    ...
    topPatterns.forEach(tp => {
      const rate = tp.acceptanceRate != null ? (tp.acceptanceRate * 100).toFixed(0) + '%' : 'â€”';
      html += `<div style="display:flex;justify-content:space-between;padding:3px 0;font-size:11px;">` +
        `<span>${esc((tp.suggestionType || tp.type || 'â€”').replace(/_/g, ' '))}</span>` +
...
// â”€â”€ Ranked Suggestions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function lrnLoadRanked() {
  ...
  el.innerHTML = ranked.map((r, i) => {
    const rate = r.acceptanceRate != null ? (r.acceptanceRate * 100).toFixed(0) : 'â€”';
    const rateCls = r.acceptanceRate >= 0.7 ? 'ok' : r.acceptanceRate >= 0.4 ? 'warn' : 'danger';
    const type = r.suggestionType || r.suggestion_type || r.type || 'â€”';
...
// â”€â”€ Archive & Feedback Loop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ INTAKE TAB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ ACI XML Intake â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function handleIntakeXmlFile(file) {
  if (!file) return;
  showIntakeXmlStatus(`Parsing ${file.name}â€¦ this may take a moment.`, 'warn');
  ...
  try {
    ...
    if (data.ok && data.caseId) {
      ...
      showIntakeXmlStatus(
        `âœ“ Case ${data.caseId} created from XML${parts.length ? ' Â· ' + parts.join(', ') : ''}`,
        'ok'
      );
      ...
      showIntakeStatus(`âœ“ XML imported: ${file.name}`, 'ok');
    } else {
      showIntakeXmlStatus(data.error || 'Failed to parse XML.', 'err');
    }
  } catch (err) {
    if (err.name === 'AbortError') {
      showIntakeXmlStatus('Timed out. Large XML files (with embedded PDF) can take a while â€” try again.', 'err');
    } else {
...
async function handleIntakeFile(file) {
  ...
  try {
    const res = await fetch(server() + '/api/intake/order', {
      method: 'POST',
      body: formData,
      headers: { 'X-API-Key': CACC_API_KEY },
    });
    ...
    if (data.ok && data.caseId) {
      ...
      showIntakeStatus(`âœ“ Case created: ${data.caseId}`, 'ok');
      // Load case in background so active case bar updates; stay on intake for review
      activeCaseId = data.caseId;
      await loadCase(data.caseId);
      // Don't auto-navigate â€” let Charles click the CTA button
    } else {
      ...
      if (code === 'PDF_EXTRACTION_FAILED' || (data.extractionMethod === 'failed')) {
        ...
        if (errLower.includes('encrypt') || errLower.includes('password') || errLower.includes('decrypt')) {
          ...
          friendlyMsg = 'Could not extract text from this PDF â€” it may be a scanned image only. Try a digitally-created PDF or contact the lender for a text-based version.';
        } else {
          friendlyMsg = 'Could not extract text from this PDF â€” it may be scanned, image-only, or encrypted. Try a different file.';
        }
        ...
        friendlyMsg = `Unrecognized order format â€” missing required fields${missing ? ': ' + missing : ''}. Is this a standard appraisal assignment sheet?`;
...
function showIntakeResult(data) {
  ...
  if (missing.length > 0) {
    html += `<div style="margin-top:10px;font-size:11px;color:var(--warn)">âš  Missing: ${missing.join(', ')}</div>`;
  }
  ...
  html += `<div style="margin-top:10px;padding:8px;background:rgba(215,179,90,.08);border:1px solid rgba(215,179,90,.25);border-radius:6px;font-size:11px;">
    <strong style="color:var(--gold)">âœ“ ${prefilled}/${total} fields pre-filled</strong>
  ...
  // Hide the manual folder button â€” folder creation is now automatic
  ...
  if(sub) sub.textContent = (extracted.address||'') + (extracted.borrowerName ? ' Â· ' + extracted.borrowerName : '');
  ...
  entry.innerHTML = `<div style="color:var(--gold);font-weight:700">${extracted.address || 'Unknown address'}</div>
    <div style="color:var(--muted)">${extracted.borrowerName || ''} Â· ${extracted.formType || ''}</div>`;
...
async function createIntakeFolder() {
  ...
  const res = await apiFetch('/api/intake/create-folder', { method: 'POST', body }).catch(() => null);
  if (res && res.ok) {
    statusEl.style.color = 'var(--ok)';
    statusEl.textContent = 'âœ“ Folder created: ' + (res.folderPath || '');
  } else {
...
async function mredLoadStatus() {
  ...
  try {
    ...
    if (data.connected) {
      ...
      body.innerHTML = '<span style="color:var(--warn);font-weight:700;">?? Credentials set ï¿½ not authorized yet</span><div style="font-size:11px;color:var(--muted);margin-top:4px;">Click Connect to authorize with MRED.</div>';
      ...
      body.innerHTML = '<span style="color:var(--danger);font-weight:700;">?? Not configured</span><div style="font-size:11px;color:var(--muted);margin-top:4px;">Add MRED_CLIENT_ID and MRED_CLIENT_SECRET to .env ï¿½ see docs/MRED_API_SETUP.md</div>';
...
// â”€â”€ Gmail Integration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function gmailSend(to, subject, body, cc = '') {
  ...
  try {
    ...
    const data = await res.json();
    if (statusEl) {
      statusEl.textContent = data.ok ? 'âœ… Email sent!' : 'âŒ ' + data.error;
      statusEl.style.color = data.ok ? 'var(--ok)' : 'var(--danger)';
      setTimeout(() => { if (statusEl) statusEl.textContent = ''; }, 5000);
    }
    return data;
  } catch (e) {
    if (statusEl) statusEl.textContent = 'âŒ ' + e.message;
...
async function wsFindPhotos() {
  ...
  try {
    ...
    // Build photo grid (filename cards ï¿½ can't show actual images without serving the files)
    if (gridEl) {
...
async function wsLoadCompGuidance() {
  ...
  try {
    ...
    if (guidEl) {
      ...
      guidEl.innerHTML = `<div style="font-size:11px;font-weight:700;color:var(--gold);margin-bottom:6px;">Comp Guidance ï¿½ Based on ${g.basedOnReports} report(s)</div>` +
        (g.glaRange ? `<div>?? <strong>GLA range:</strong> ${g.glaRange.min.toLocaleString()}ï¿½${g.glaRange.max.toLocaleString()} sf (ï¿½15% of subject)</div>` : '') +
  ...
  if (isHidden) {
    ...
    try {
      ...
      if (st) st.innerHTML = data.connected
        ? '<span style="color:var(--ok);">?? MRED connected ï¿½ live search available</span>'
        : '<span style="color:var(--warn);">?? MRED not connected ï¿½ <a href="#" onclick="showTab(\'system\')">go to System tab to connect</a></span>';
    } catch (e) {
...
async function wsCompCsvUpload(file) {
  ...
  try {
    ...
    const geoNote = data.geocoded > 0 ? ` ï¿½ ${data.geocoded} geocoded` : '';
    if (statusEl) { statusEl.style.color = 'var(--ok)'; statusEl.textContent = `? ${data.count} comp(s) imported${geoNote} ï¿½ saved to case`; }
...
function _wsRenderCompsTable(comps, label) {
  ...
  tbody.innerHTML = comps.map((c, i) => {
    const glaColor = c.glaDiff && c.glaDiff !== 'ï¿½'
      ? (parseFloat(c.glaDiff) > 15 ? 'color:var(--warn)' : 'color:var(--ok)')
      : '';
    return `<tr style="border-bottom:1px solid rgba(255,255,255,.04);">
      <td style="padding:5px 8px;">${esc(c.address || 'ï¿½')}</td>
      <td style="text-align:right;padding:5px 8px;color:var(--gold);">${esc(c.priceDisplay || 'ï¿½')}</td>
      <td style="text-align:right;padding:5px 8px;">${esc(c.glaDisplay || 'ï¿½')}</td>
      <td style="text-align:right;padding:5px 8px;${glaColor}">${esc(c.glaDiff || 'ï¿½')}</td>
      <td style="text-align:right;padding:5px 8px;color:var(--muted);">${esc(c.pricePerSf || 'ï¿½')}</td>
      <td style="text-align:right;padding:5px 8px;">${c.beds || 'ï¿½'}/${c.baths || 'ï¿½'}</td>
      <td style="text-align:right;padding:5px 8px;color:var(--muted);">${esc(c.saleDate || 'ï¿½')}</td>
      <td style="text-align:right;padding:5px 8px;color:var(--muted);">${esc(c.distDisplay || 'ï¿½')}</td>
    </tr>`;
  }).join('');
