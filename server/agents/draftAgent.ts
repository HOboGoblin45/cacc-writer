...
import { retrieveExamples, type RetrievedExample } from '../retrieval/llamaIndex.js';
 ...
 */
export async function draftSection(state: WorkflowState): Promise<DraftOutput> {
...
async function _draftSection(state: WorkflowState): Promise<DraftOutput> {
  const start = Date.now();
  const { caseId, formType, fieldId, facts, locationContext } = state;
 ...
 */
function buildFactsSummary(facts: Record<string, unknown> | undefined, fieldId: string): string {
  if (!facts) return fieldId;

  const parts: string[] = [`field: ${fieldId}`];

  // Extract key facts for the query
  const subject = (facts.subject as Record<string, any>) || {};
  const market  = (facts.market  as Record<string, any>) || {};
  const neigh   = (facts.neighborhood as Record<string, any>) || {};

  const getValue = (obj: Record<string, any>, key: string): string => {
    const f = obj[key];
  ...
  if (trend)  parts.push(`market trend: ${trend}`);
