...
export class RealQuantumTool {
   ...
   */
  async health(): Promise<RQHealthResult> {
    try {
   ...
   */
  async navigateSection(sectionName: string): Promise<RQToolResult> {
    try {
   ...
   */
  async resolveEditor(selector: string): Promise<RQToolResult> {
    try {
   ...
   */
  async insertText(fieldId: string, text: string, formType = 'commercial'): Promise<RQInsertResult> {
    const start = Date.now();
    try {
      const res = await this._fetch('/insert', 'POST', { fieldId, text, formType });

      const result: RQInsertResult = {
        ok:          Boolean(res.ok),
      ...
      const result = this._handleError(err) as RQInsertResult;
      await logAutomation({
        caseId:     'workflow',
        fieldId,
        formType,
   ...
   */
  async readText(fieldId: string, formType = 'commercial'): Promise<RQReadResult> {
    try {
      ...
      return this._handleError(err) as RQReadResult;
    }
  }
   ...
   */
  async verifyText(fieldId: string, expected: string, formType = 'commercial'): Promise<RQVerifyResult> {
    const start = Date.now();
    try {
      ...
      return this._handleError(err) as RQVerifyResult;
    }
  }
   ...
   */
  async insertAndVerify(params: RQInsertParams): Promise<InsertionResult> {
  ...
  private _handleError(err: any): RQToolResult {
