...
export const PRODUCTION_LANE_FIELDS: Record<string, string[]> = {
  '1004': [
...
export async function parseDocument(
  ...
  for (const fieldId of targetFields) {
    ...
    if (!text || typeof text !== 'string' || text.length < 20) continue;
...
function propertyTypeFromForm(formType: string): string {
  switch (formType) {
