# CACC Writer — Production-Hardening Implementation Plan
...
### The core problem — the split brain
The production server (`cacc-writer-server.js`) and the modular server (`server/server.js`) are **completely disconnected**. The production server has its own inline `collectExamples()`, its own `genInput()`, its own OpenAI client — none of which use the modular KB, retrieval, or prompt pipeline. The modular server runs on port 5179 and is **never called by anything**. The knowledge base has **0 examples** because the feedback loop saves to `feedback.json` but never calls `addExample()`. The two-pass review, confidence gating, and queue workflow do not exist yet.

**The fix is surgical, not a rewrite.** The modular modules are good — they just need to be imported into the production server and wired up. Everything else builds on top of that.

---

## ACTIVE PRODUCTION SCOPE

> ⚠️ **Scope is narrowed.** Active production lanes: **1004 single-family (ACI)** + **commercial (Real Quantum)** only.
> 1025, 1073, 1004c are **DEFERRED** — preserved, not actively extended. See `SCOPE.md` for full definition.
>
> **Scope enforcement is implemented** across API + UI:
> - New cases / generation / workflow for deferred forms → **BLOCKED** (`{supported:false, scope:'deferred'}`)
...
|-------|-------|----------|-------------|------------|-------|
| 1 | Runtime Architecture Consolidation | 🔴 Critical | 1–2 days | None | Both lanes |
| 2 | Knowledge Base Activation | 🔴 Critical | 1 day | Phase 1 | Both lanes |
| 3 | Retrieval & Prompt Pipeline Completion | 🔴 Critical | 1 day | Phase 1, 2 | Both lanes |
| 4 | Two-Pass Draft / Review Workflow | 🟠 Important | 1–2 days | Phase 3 | Both lanes |
| 5 | Fact Confidence Gating & Safety Rules | 🟠 Important | 1 day | Phase 3 | Both lanes |
| 6 | ACI Automation Hardening | 🟠 Important | 1–2 days | Phase 1 | **1004 only** |
| 7 | Real Quantum Automation Hardening | 🟡 Important | 1–2 days | Phase 6 | **commercial only** |
| 8 | Throughput & Queue Workflow | 🟡 Important | 2 days | Phase 4 | Both lanes |
| 9 | Testing, Logging, Error Handling | 🟠 Important | 1–2 days | Phase 8 | Both lanes |
| 10 | Final Production Readiness Audit | 🟡 Optional | 0.5 day | All | Both lanes |

**Total realistic timeline: 12–18 working days (2.5–3.5 weeks)**
...
### PHASE 1 — Runtime Architecture Consolidation
...
- `collectExamples()` — reads `voice_training.json` + case `feedback.json` (NOT the KB)
- `genInput()` — just prepends system prompt + style guide (NOT the full pipeline)
- `const client = new OpenAI(...)` — its own OpenAI client instance

`server/server.js` runs standalone on port 5179 and is never called by anything.

#### Tasks

**1.1 — CRITICAL: Import modular modules into cacc-writer-server.js**
...
**1.2 — CRITICAL: Replace `genInput()` with `buildPromptMessages()`**
...
**1.3 — CRITICAL: Replace `collectExamples()` with `getRelevantExamples()`**
...
- Keep `voice_training.json` as a deprecated data source — add a migration path in Phase 2.
- Mark `collectExamples()` as `// DEPRECATED — use getRelevantExamples() instead`

**1.4 — IMPORTANT: Deprecate `server/server.js` as a standalone server**
...
- Do NOT delete it — the modules it imports are still used.

**1.5 — IMPORTANT: Unify the OpenAI client**
...
#### Files to edit
- `cacc-writer-server.js` — primary target (add imports, replace genInput, replace collectExamples)
- `server/server.js` — add deprecation notice, guard the listen block
...
```
1. npm start — server starts on port 5178 with no import errors
2. POST /api/generate with a simple prompt — returns text
3. POST /api/generate-batch with 2 fields — both return text
4. Console shows no duplicate client warnings
5. GET /api/health — returns { ok: true }
```

#### Risk notes
- The `callAI()` function uses `client.responses.create()` — same API as the inline client. Drop-in replacement.
- OCR vision calls use a different input format — keep the raw client for those specifically.
- `buildPromptMessages()` expects `fieldId` as a parameter — ensure all generation calls pass it.

---

### PHASE 2 — Knowledge Base Activation
...
**2.1 — CRITICAL: Wire feedback endpoint to save approved edits to KB**
...
**2.2 — CRITICAL: Create voice training migration endpoint**
...
- Idempotent — skip entries already in the KB

**2.3 — IMPORTANT: Add KB management API endpoints**
- `GET /api/kb/status` — return counts from `index.json`
- `POST /api/kb/reindex` — rebuild index from disk (move from `server/server.js`)

**2.4 — IMPORTANT: Extend voice PDF import to also save to KB**
...
**2.5 — OPTIONAL: Add KB phrase management endpoints**
- `GET /api/kb/phrases` — return all phrases
- `POST /api/kb/phrases` — add a new phrase
- `DELETE /api/kb/phrases/:id` — remove a phrase

#### Files to edit
- `cacc-writer-server.js` — feedback endpoint, new KB endpoints, voice import extension
...
2. Check knowledge_base/approved_edits/ — a new .json file should appear
3. Check knowledge_base/index.json — counts.approved_edits should be > 0
4. GET /api/kb/status — returns non-zero counts
5. Generate the same field again — the approved edit should appear in the examples block
6. POST /api/kb/migrate-voice — migrates voice_training.json entries
```

---

### PHASE 3 — Retrieval and Prompt Pipeline Completion

**Objective:** Every generation request uses the full structured pipeline: system prompt → style guide → retrieved examples → phrase bank → facts → user request.
...
**3.1 — CRITICAL: Update `generate-batch` to use `buildPromptMessages()`**
...
**3.2 — CRITICAL: Update `POST /api/generate` to use `buildPromptMessages()`**

Update the single-field generate endpoint to accept `fieldId`, `formType`, `facts` and use `buildPromptMessages()`.

**3.3 — IMPORTANT: Complete `FIELD_LABELS` in `promptBuilder.js`**
...
**3.4 — IMPORTANT: Complete `FIELD_PHRASE_TAGS` in `promptBuilder.js`**
...
**3.5 — IMPORTANT: Inject form-specific `tpl` instructions into `buildPromptMessages()`**
...
**3.6 — OPTIONAL: Add retrieval fallback logging**
...
#### Files to edit
- `cacc-writer-server.js` — update generate and generate-batch endpoints
- `server/promptBuilder.js` — add FIELD_LABELS, FIELD_PHRASE_TAGS completeness, form tpl injection
- `server/retrieval.js` — add fallback logging
...
2. Log the messages array sent to OpenAI — verify all 6 blocks are present
3. Approve an edit, then generate the same field again — verify the approved edit appears
...
### PHASE 4 — Two-Pass Draft / Review Workflow

**Objective:** Implement a draft → review → approve pipeline that catches unsupported claims, tone issues, and missing facts before the appraiser sees the output.
...
**4.1 — CRITICAL: Create `prompts/review_pass.txt`**

New file with the reviewer system prompt:
```
You are a senior appraisal reviewer for CACC Writer.

Check the draft narrative for:
1. Unsupported factual claims — anything stated as fact not in the provided facts
2. Overconfident language — certainty where support is limited
3. Missing placeholders — facts that should be [INSERT] but were invented
4. Tone issues — speculative, academic, or non-professional language
5. USPAP compliance — avoid unsupported conclusions
6. Internal contradictions — statements that conflict with each other or the facts
...
**4.2 — CRITICAL: Add `POST /api/cases/:caseId/review-section` endpoint**
...
**4.3 — IMPORTANT: Add `buildReviewMessages()` to `promptBuilder.js`**
...
**4.4 — IMPORTANT: Add `twoPass` option to `generate-batch`**

```js
// In generate-batch request body:
// twoPass: true  — run draft then review, return reviewed text
// twoPass: false — return draft as-is (default, faster)
```

**4.5 — IMPORTANT: Update UI to show review results**
...
**4.6 — OPTIONAL: Auto-review for high-stakes fields**
...
#### Files to create
- `prompts/review_pass.txt` — new reviewer system prompt

#### Files to edit
- `cacc-writer-server.js` — new review-section endpoint, twoPass option in generate-batch
- `server/promptBuilder.js` — add `buildReviewMessages()` function
- `app.js` — UI review button and issues display
...
5. Test two-pass batch generation — verify it takes ~2x longer but produces better output
6. Test auto-review for reconciliation field
```

---

### PHASE 5 — Fact Confidence Gating and Safety Rules
...
**5.1 — CRITICAL: Update `buildFactsContext()` to annotate confidence**
...
L.push('  address: 123 Main St [confidence: medium — use hedged language]');

// For low confidence: replace with INSERT
L.push('  address: [INSERT] [confidence: low — do not state as fact]');
```

Logic:
- `confidence: 'high'` → output value as-is, no annotation
- `confidence: 'medium'` → output value with `[confidence: medium — use hedged language]`
- `confidence: 'low'` → output `[INSERT]` with `[confidence: low — do not state as fact]`
- `null` value → output `[INSERT]` regardless of confidence

**5.2 — CRITICAL: Update `prompts/system_cacc_writer.txt` with confidence rules**

Add to the system prompt:
```
CONFIDENCE RULES (strictly enforced):
- Facts with no annotation: state as fact
- Facts marked [confidence: medium]: use hedged language — "reportedly", "per available records", "indicated as", "per the [source]"
- Facts marked [confidence: low]: write [INSERT] or omit entirely — NEVER state as certain
...
**5.3 — IMPORTANT: Update `buildPromptMessages()` facts block to match**
...
**5.4 — IMPORTANT: Add confidence check to review pass**
...
**5.5 — OPTIONAL: Confidence upgrade on approval**
...
#### Files to edit
- `cacc-writer-server.js` — `buildFactsContext()` function
- `server/promptBuilder.js` — facts block formatting
- `prompts/system_cacc_writer.txt` — add confidence rules
- `prompts/review_pass.txt` — add confidence check (Phase 4 dependency)
...
2. Generate a section — verify output uses [INSERT] or hedged language throughout
3. Set one fact to confidence: 'high', regenerate — verify that fact is stated directly
4. Run review pass — verify it flags any remaining overconfident statements
5. Verify medium-confidence facts use hedged language ("reportedly", "indicated")
```

---

### PHASE 6 — ACI Automation Hardening
...
**6.1 — CRITICAL: Add `GET /calibrate` endpoint to ACI agent**
...
**6.2 — CRITICAL: Add `automation_id` support to field maps and insertion strategy**
...
**6.3 — CRITICAL: Add `POST /test-field` endpoint**
...
**6.4 — IMPORTANT: Add screenshot-on-failure**
...
**6.5 — IMPORTANT: Add `POST /insert-batch` endpoint**
...
**6.6 — IMPORTANT: Escalating strategy retry**
...
#### Files to edit
- `desktop_agent/agent.py` — calibrate, test-field, batch insert, screenshot-on-failure, automation_id strategy
- `desktop_agent/field_maps/1004.json` — add automation_id fields after calibration
- `desktop_agent/field_maps/1025.json` — same
- `desktop_agent/field_maps/1073.json` — same
- `desktop_agent/config.json` — add screenshot_dir setting
...
2. GET http://localhost:5180/calibrate — verify it returns control list with automation_ids
3. Update field_maps/1004.json with real automation_ids from calibration output
4. POST /test-field for neighborhood_description — verify test string appears in ACI
5. POST /insert with real text — verify it inserts and verifies correctly
6. Test clipboard fallback by temporarily breaking the label match
7. POST /insert-batch with 3 fields — verify all 3 insert sequentially
```

#### Risk notes
- ACI's Win32 control structure may vary by version. The calibration endpoint is essential before any other ACI work.
- Some ACI fields may be in child windows or dialogs — the agent may need to handle window switching.
- Run calibration with a real 1004 report open, not just the ACI main window.

---

### PHASE 7 — Real Quantum Automation Hardening
...
**7.1 — CRITICAL: Run selector discovery on a live Real Quantum session**
...
1. `income_approach` — highest value for commercial
2. `reconciliation` — highest value for commercial
3. `site_description` — commonly needed
4. `hbu_analysis` — commonly needed
5. `sales_comparison` — commonly needed

Update `real_quantum_agent/field_maps/commercial.json` with real selectors after discovery.

**7.2 — CRITICAL: Add screenshot-on-failure to RQ agent**
...
**7.3 — IMPORTANT: Add `POST /test-field` endpoint to RQ agent**

Same concept as ACI: insert a test string, verify, clear. Essential for validating selectors.

**7.4 — IMPORTANT: Add `POST /insert-batch` endpoint to RQ agent**

Sequential batch insert for commercial fields.

**7.5 — IMPORTANT: Add configuration-driven navigation timeout**
